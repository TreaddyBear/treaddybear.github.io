...
import { createFallenLeaves } from "./fallenLeaves";
...
// Remember the player's last real level for the menu, but start the scene on the
// attract-only showcase map so the opening flyby films the showcase, not whatever
// level was last played. Picking "Start/Continue" loads a real playable level.
settings.mapId = savedMenuPreferences.lastLevelCode ?? settings.mapId;
settings.mapId = showcaseLevelCode;
...
const fallenLeaves = createFallenLeaves(scene, groundHeightAt);
...
// Shared with the attract-wipe DOM helper below.
...
function resetGame() {
  applyActiveMap();
  // The attract showcase is a clean, isolated open field — an "empty field full
  // of promises". Hide everything that isn't grass/flowers/clover: the parked
  // mower, the scenery trees + rocks, the hidden gun prop, and the neighbourhood
  // road (its east edge clips the field).
  ...
  secretGunRoot?.setEnabled(!isShowcase);
  grass.generate();
  dandelions.place();
  tulips.place();
  fieldFlowers.place();
...
const treesRoot = createSimpleTrees(scene, materials, shadowGenerator);
const scenery = createSceneryRocks(scene, materials, shadowGenerator);
...
syncGameplayInputVisibility = () => {
  ...
  fallenLeaves.setActive(menu.isOpen() && !gameStarted);
};
